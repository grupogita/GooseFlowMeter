...
from .goose.goose_data import get_payload_length
...
class PacketCount:
    ...
    def get_payload(packet):
        return get_payload_length(packet)
        ...
        if packet_direction is not None:
            return len(
                [
                    ...
                    for packet, direction in self.feature.packets
                    if direction == packet_direction
                    and self.get_payload(packet) > 0 # len of int 
                ]
            )
        return len(
            [
                packet
                for packet, direction in self.feature.packets
                if self.get_payload(packet) > 0 # len of int 
            ]
        )
